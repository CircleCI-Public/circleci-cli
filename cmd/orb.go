...
Please note that at this time all orbs created in the registry are world-readable.`,
	...
	orbInit := &cobra.Command{
		Use: "init <orb name>",
		Short: "Initialize a new orb using the Orb starter kit",
		Long: `Initialize a new orb using the Orb starter kit`,
		RunE: func(_ *cobra.Command, _ []string) error {
			return initOrb(opts)
		},
		Args: cobra.ExactArgs(1),
...
func initOrb(opts orbOptions) error {
	...
	fmt.Printf("Downloading orb-starter-kit\n")

	fileURL := "https://github.com/scwheele/orb-starter-kit/archive/master.zip"
	...
	out, err := os.Create("orb-starter-kit.zip")
	if err != nil {
		return err
	}
	defer out.Close()
	...
	fmt.Printf("Unzipping orb-starter-kit\n")
	err = Unzip("orb-starter-kit.zip", "orb-starter-kit")
	if err != nil {
		return err
	}

	os.Rename("orb-starter-kit/orb-starter-kit-master", name)
	fmt.Printf("Cleaning up directory\n")

	os.Remove("orb-starter-kit")
	os.Remove("orb-starter-kit.zip")

	fmt.Printf("Created Orb called %s\n", name)
